...
int main() {
    ...
    string_col[1] = df::NA_VALUE;
    
    std::vector<std::pair<std::string, df::ColumnData>> data = {
        {"integers", int_col},
        {"doubles", double_col},
        {"strings", string_col},
        {"booleans", bool_col}
    ...
    std::vector<std::pair<std::string, df::ColumnData>> data2 = {
        {"integers", int_col2},
        {"doubles", double_col2}
    };
    ...
    std::cout << "Reading CSV back from " << test_file << std::endl;
    df::DataFrame read_back = df::io::read_csv(test_file);
    std::cout << "Read-back DataFrame:" << std::endl;
    read_back.display();

    if (read_back.numColumns() == dataframe.numColumns()) {
        std::cout << "Column count matches: " << read_back.numColumns() << std::endl;
    } else {
        std::cout << "ERROR: Column count mismatch! Expected "
                  << dataframe.numColumns() << " but got "
                  << read_back.numColumns() << std::endl;
    }

    if (read_back.numRows() == dataframe.numRows()) {
        std::cout << "Row count matches: " << read_back.numRows() << std::endl;
    } else {
        std::cout << "ERROR: Row count mismatch! Expected "
                  << dataframe.numRows() << " but got "
                  << read_back.numRows() << std::endl;
    }
